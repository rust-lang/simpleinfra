...
Dedicated Host auto recovery is enabled for M9g, allowing EC2 to replace the
physical host after supported power or network failures. It does not run for a
scheduled host retirement, so operators must still act on EC2 maintenance and
