...
/// cached files of a crate can be purged with a single request. These keys are added to the keys
/// already generated by Fastly, which are needed for URL purges. The header is removed from the
    ...
    request.set_after_send(|candidate| {
        if candidate.get_status().is_success() {
            if let Some(tags) = candidate.remove_header_str("x-amz-meta-cache-tags") {
                let existing_surrogate_keys: Vec<String> = candidate
                    .get_surrogate_keys()
                    .filter(|key| !key.is_empty())
                    .map(String::from)
                    .collect();

                let new_surrogate_keys = existing_surrogate_keys
                    .iter()
                    .map(String::as_str)
                    .chain(parse_cache_tags(&tags));

                candidate.set_surrogate_keys(new_surrogate_keys);
            }
        }
        Ok(())
    });
