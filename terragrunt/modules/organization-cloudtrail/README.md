...
This module creates one multi-Region organization trail that records read and
...
[`log-lambda-forwarder-datadog` Terraform module][forwarder-module]. Its outputs
are consumed by the [`datadog-aws`](../datadog-aws) stack, which registers the
Forwarder and enables CloudTrail log auto-subscription. This lets Datadog
configure the S3 notification that invokes the Forwarder for new log files.
Forwarded logs have the `cloudtrail` source and can be analyzed by Cloud SIEM.
